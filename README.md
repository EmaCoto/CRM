...
You can run this project locally with Docker Compose using PHP-FPM, Nginx, MySQL, Redis, and Mailpit.
...
2. Install PHP dependencies inside Docker:
...
```
docker compose run --rm artisan krayin-crm:install
```
...
If you want to run Vite in watch mode for frontend work:

```
docker compose --profile vite up node
```
